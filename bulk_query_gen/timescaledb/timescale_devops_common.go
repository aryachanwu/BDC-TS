...
func (d *TimescaleDevops) maxCPUUsageHourByMinuteNHosts(qi bulkQuerygen.Query, nhosts int, timeRange time.Duration) {
	...
	q.QuerySQL = []byte(fmt.Sprintf("select time_bucket2(time,60000000000) , max(usage_user) from cpu where (%s) and time >=%d and time < %d group by time_bucket2(time,60000000000) order by time_bucket2(time,60000000000)", combinedHostnameClause, interval.StartUnixNano(), interval.EndUnixNano()))
}
...
func (d *TimescaleDevops) MeanCPUUsageDayByHourAllHostsGroupbyHost(qi bulkQuerygen.Query) {
	...
	q.QuerySQL = []byte(fmt.Sprintf("select avg(usage_user) from cpu where time >=%d and time < %d group by time_bucket2(time, 360000000000),hostname order by time_bucket2(time, 360000000000)", interval.StartUnixNano(), interval.EndUnixNano()))
}
