...
func (d *TimescaleDevops) maxCPUUsageHourByMinuteNHosts(qi bulkQuerygen.Query, nhosts int, timeRange time.Duration) {
	...
	q.QuerySQL = []byte(fmt.Sprintf("select max(usage_user) from cpu where (%s) and time >=%d and time < %d group by time order by time ", combinedHostnameClause, interval.StartUnixNano(), interval.EndUnixNano()))
}
...
func (d *TimescaleDevops) MeanCPUUsageDayByHourAllHostsGroupbyHost(qi bulkQuerygen.Query) {
	...
	q.QuerySQL = []byte(fmt.Sprintf("select avg(usage_user) from cpu where time >=%d and time < %d group by time,hostname order by time", interval.StartUnixNano(), interval.EndUnixNano()))
}
